drop table DimAgencyDetails
drop table DimPolicyDetails
drop table DimPeriod
drop table DimProd

SELECT ROW_NUMBER() OVER ( ORDER BY [AGENCY_ID] ) AS AGENCY_DETAILS_ID, AgencyDetails.*
...
SELECT ROW_NUMBER() OVER ( ORDER BY [CL_BOUND_CT_MDS]) AS POLICY_DETAILS_ID, PolicyDetails.*
...
SELECT ROW_NUMBER() OVER ( ORDER BY [STAT_PROFILE_DATE_YEAR], [MONTHS] ) AS PERIOD_ID, PeriodDetails.*
INTO DimPeriod
FROM
(
	SELECT DISTINCT 
...
SELECT ROW_NUMBER() OVER ( ORDER BY [PROD_ABBR]
		,[PROD_LINE]
...
SELECT ROW_NUMBER() OVER ( ORDER BY [VENDOR_IND], [VENDOR] ) AS VENDOR_ID, VendorDetails.*
INTO DimVendor
FROM
(
	SELECT DISTINCT 
